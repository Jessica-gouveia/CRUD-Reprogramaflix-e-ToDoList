...
const getSerieByTitle = (request, response) => {
    ...
    if (requiredTitle === "" || titleFiltered === undefined) {
        response.status(404).send(
            {
                "message": "Opa! Insira um título válido por favor =) "
            }
        )
    } else {
...
const createSerie = (request, response) => {
    ...
    response.status(200).send({"message": "Série criada com sucesso!!", newSerie})
}
...
const deleteSerie = (request, response) => {
    ...
    response.status(200).send({"message": "Série deletada com sucesso!", series})
...
const updateSerieTitle = (request, response) => {
    ...
    response.status(200).send({
        "message": "Título atualizado com sucesso!", filteredIdSerie
    })
}
...
const updateAnythingSerie = (request, response) => {
    ...
    response.status(200).send({
        "message": "Informação atualizada com sucesso!", filteredIdSerie
    })
}
